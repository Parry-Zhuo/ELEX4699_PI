...
void CMotor::changeFreq(int newFreq) {
    // Return right PWM value
    //_control._pinleftPWM = newFreq;
    //_control._pinleftPWM = newFreq;
    gpioSetPWMfrequency(_control._pinleftPWM, newFreq);
    gpioSetPWMfrequency(_control._pinrightPWM, newFreq);
}
void CMotor::stop() {
    // Stop motors
    _leftPWM = 0;
...
void CMotor::look(int direction){
//    std::this_thread::sleep_until(std::chrono::system_clock::now() + std::chrono::milliseconds(50));
    if(direction == -1){
        _control.servoControl(_control._pinCameraRotation, 180);
    }else if (direction == 0){
        _control.servoControl(_control._pinCameraRotation, 90);
    }else if (direction == 1){
        _control.servoControl(_control._pinCameraRotation, 0);
    }
    //Look straight
}
